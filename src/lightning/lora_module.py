...
class ModelWrapper(pl.LightningModule):
    def __init__(self, inner_model):
        ...
        if hasattr(inner_model, "named_parameters"):
...
class LoraLightningModule(pl.LightningModule):
    def __init__(self, trainer):
        ...
        if hasattr(trainer, "model"):
            if isinstance(trainer.model, torch.nn.Module):
                self.model = trainer.model
            else:
                self.model = ModelWrapper(trainer.model)
        else:
            self.model = None
            logger.warning("Trainer has no 'model' attribute")

        self._trainer_config = (
            dict(trainer.config) if hasattr(trainer, "config") else {}
        )
        serializable_config = {
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        return self.trainer_module.process_batch(
            batch, self.trainer_module.train_metrics
        )
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        processed_batch = self.trainer_module.process_batch(
            batch_with_idx, self.trainer_module.train_metrics
        )
        ...
        if batch_idx % self.trainer_module.log_step == 0:
            ...
            if hasattr(self.trainer_module, "writer") and self.trainer_module.writer:
                ...
                self.trainer_module._log_batch(
                    processed_batch, None, batch_idx, "train", self.current_epoch
                )
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> Dict[str, Any]:
        if batch_idx == 0:
            self.trainer_module._update_pipeline_with_current_weights(
                self.current_epoch
            )

        processed_batch = self.trainer_module.process_evaluation_batch(
            batch, self.trainer_module.evaluation_metrics, is_training_mode=False
        )

        if "loss" in processed_batch:
            self.log(
                "val_loss",
                processed_batch["loss"],
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True,
